...
export type ColorResult = { ok: true; value: { hex: string; r: number; g: number; b: number } }
...
export function hexToRgb(input: HexInput): ColorResult {
  ...
  if (!raw) {
    return { ok: false, error: { code: "EMPTY", message: "请输入 HEX 值（如 #2563eb）" } };
  }
  const stripped = raw.replace(/^#/, "");
  if (!HEX6.test(stripped) && !HEX3.test(stripped)) {
    return { ok: false, error: { code: "INVALID_HEX", message: "HEX 必须是 3 或 6 位十六进制字符" } };
  }
...
export function rgbToHex(input: RgbInput): ColorResult {
  ...
  for (const [name, val] of [["R", r], ["G", g], ["B", b]] as const) {
    if (!Number.isInteger(val) || val < 0 || val > 255) {
      return { ok: false, error: { code: "INVALID_RGB", message: `${name} 值需在 0~255 整数之间` } };
    }
  }
