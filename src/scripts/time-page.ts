// 时间换算计算器页面交互（外部脚本，无内联事件；样板见 _page-kit.ts）
import { convertTime, timeUnit } from "../lib/calculators/time";
import {
  requireEl,
  setFieldError,
  clearFieldError,
...
const unitText = (code: string): string => timeUnit(code)?.symbol ?? code;
