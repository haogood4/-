// 存储容量换算计算器页面交互（外部脚本，无内联事件；样板见 _page-kit.ts）
import { convertStorage, storageUnit } from "../lib/calculators/data-storage";
import {
  requireEl,
  setFieldError,
  clearFieldError,
...
const unitText = (code: string): string => storageUnit(code)?.symbol ?? code;
