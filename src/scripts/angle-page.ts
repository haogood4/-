// 角度单位换算计算器页面交互（外部脚本，无内联事件；样板见 _page-kit.ts）
import { convertAngle, angleUnit } from "../lib/calculators/angle";
import {
  requireEl,
  setFieldError,
  clearFieldError,
...
const unitText = (code: string): string => angleUnit(code)?.symbol ?? code;
