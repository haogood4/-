// 速度换算计算器页面交互（外部脚本，无内联事件；样板见 _page-kit.ts）
import { convertSpeed, speedUnit } from "../lib/calculators/speed";
import {
  requireEl,
  setFieldError,
  clearFieldError,
...
const unitText = (code: string): string => speedUnit(code)?.symbol ?? code;
