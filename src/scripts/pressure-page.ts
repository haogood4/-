// 压力单位换算计算器页面交互（外部脚本，无内联事件；样板见 _page-kit.ts）
import { convertPressure, pressureUnit } from "../lib/calculators/pressure";
import {
  requireEl,
  setFieldError,
  clearFieldError,
...
const unitText = (code: string): string => pressureUnit(code)?.symbol ?? code;
