// URL 编码解码页面交互（外部脚本，无内联事件；样板见 _page-kit.ts）
import { convertUrl, type UrlMode } from "../lib/calculators/url-codec-cn";
import {
  requireEl,
  setFieldError,
  clearFieldError,
...
const MODE_LABEL: Record<UrlMode, string> = {
  encode: "编码",
  decode: "解码",
};

function apply(mode: UrlMode): void {
  clearFieldError(textInput, errorText);
  const r = convertUrl({ input: textInput.value, mode });
  if (!r.ok) {
...
modeButtons.forEach((btn) => {
  btn.addEventListener("click", () =>
    apply((btn.dataset.mode ?? "encode") as UrlMode),
  );
});
