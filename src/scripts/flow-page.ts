// 流量单位换算计算器页面交互（外部脚本，无内联事件；样板见 _page-kit.ts）
import { convertFlow, flowUnit } from "../lib/calculators/flow";
import {
  requireEl,
  setFieldError,
  clearFieldError,
...
const unitText = (code: string): string => flowUnit(code)?.symbol ?? code;
