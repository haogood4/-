// 体积换算计算器页面交互（外部脚本，无内联事件；样板见 _page-kit.ts）
import { convertVolume, volumeUnit } from "../lib/calculators/volume";
import {
  requireEl,
  setFieldError,
  clearFieldError,
...
const unitText = (code: string): string => volumeUnit(code)?.symbol ?? code;
