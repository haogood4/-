...
- [x] **CSS gzip ≤ 30KB**、**JS gzip ≤ 100KB**（`pnpm bundle:check` 通过；当前 6.19KB / **59.75KB**（P2-9 +0.96KB、P2-10 +2.44KB），重回 65KB 软红线内 ✅——去重专项两阶段完成（2026-09-20）：①9 页样板提取至 `src/scripts/_page-kit.ts`（esm+splitting 共享 chunk）；②其余 42 页经 codemod `scripts/migrate-pages-to-kit.mjs` 全量迁移至 kit 并统一外置管线（`build-public-scripts.mjs` 自动发现全部 `*-page.ts`，页面一律 `<script is:inline type="module" src="/scripts/x-page.js">`），dist 已无 Astro 打包 JS，样板拷贝 51 份→共享 chunk 数份；新增代码不得再使 JS 增长）
...
4. **用户体验**：暗色模式（token 已就绪，补 `[data-theme="dark"]` 变量覆盖 + 切换按钮 + localStorage 持久化 + 无闪烁内联判定脚本需 CSP 豁免评估）、站内搜索（构建期 JSON 索引 + 前端模糊匹配，零依赖）、收藏/最近使用（localStorage）
...
- [ ] P1-7 暗色模式（方案先行）｜预估 1 天｜📋 2026-09-20 方案已产出待批复：**推荐分两期**——①先落地纯 CSS 自动跟随（`@media (prefers-color-scheme: dark)` 覆盖 :root 约 22 个颜色变量，零 JS/零 CSP 影响/无 FOUC；同步 color-scheme meta 改 "light dark"、theme-color 双变体、manifest theme_color）；②手动切换开关（localStorage + 外置 theme-init.js 走既有管线，需人工确认是否引入——内联早置脚本违反 CSP 红线，必须外置，存在 FOUC 窗口）。改造面勘察：global.css 181 处 var(--color-) 引用，硬编码色仅 1 处（#fff 于深色底文字，天然兼容）+ 品牌 SVG 白底（合理保留），全部状态色（danger/warning-bg 等）均为变量，覆盖清单可控。验收：暗色下全站 WCAG AA 4.5:1（primary 提亮为 teal-400 系）、bundle CSS 增量 <1KB gzip
