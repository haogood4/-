...
7. **质量门禁**：`pnpm verify` = typecheck（strict）+ eslint（0 error 门禁，`no-explicit-any`=error）+ vitest 781 用例 + prettier + mcp:check + bundle:check（**每页 JS 预算模型**：page script+其 chunk ≤8KB / 全站公共脚本 ≤4KB / 全站 JS 总和护栏 ≤120KB / CSS ≤30KB，brotli 口径）；`pnpm verify:dist` 追加 build 后 12 组冒烟断言
...
- [x] **体积门禁（2026-09-21 起：每页 JS 预算模型）**：`pnpm bundle:check` 四项——A. 每页 JS（page script + 其静态 import chunk，递归解析）≤ **8KB** 硬限 / 7KB 预警（当前最差 qr-code-cn 6.17KB）；B. 全站公共脚本（theme+menu+register-sw）≤ **4KB**（当前 2.14KB）；C. 全站 JS 总和 ≤ **120KB** 仅作数据内联回归护栏（当前 95.07KB；P3 批次 +9.34KB 中含 kit 增长——新页复用 bindCopyButton 等使共享 chunk 变大，单页实载仍 ≤1.9KB，护栏余量 24.93KB ≈ 再 2-3 个批次）；D. CSS ≤ 30KB（当前 8.87KB）。**口径迁移理由（体积专项结论）**：多页纯静态架构下每页只加载自身脚本+kit，「全站总和」与真实性能无关且与持续加工具路线架构性冲突；审计证实无剩余削减空间（83 文件最大 3.29KB 纯算法、数据大户已外置完毕、4 个 kit chunk 零孤儿），「按分类 code-splitting」经评估**不减总量属无效重构，否决**。历史：P0/P1/P2 三批次共 +29KB 均为不可去重业务逻辑（明细见 git 3.2 节历史版本）；**体积削减专项完成（2026-09-21）**：繁简字库/敏感词词库/服装尺码表三工具数据外置为 `public/data/*.json`（不计 JS 预算，走 HTTP/SW 缓存），页面脚本首次操作时 fetch 懒加载（模块级 Promise 缓存 + 失败 setFieldError 提示），引擎改为数据参数化纯函数；实测 brotli 84.19→79.00KB（−5.19KB，敏感词/尺码表净省低于预期，因数据本身高压缩率）；browser_use 实测三工具懒加载链路全通过（繁简双向/敏感词命中/尺码四制对照 + /data JSON 200 + console 无错）；smoke 断言 8 新增外置 JSON 存在+可解析守卫；**后续含大数据表的工具默认采用此数据外置模式**。**口径修正（2026-09-20）**：门禁由 gzip 切为 **brotli**（生产 Cloudflare Pages 默认下发 brotli，gzip 高估约 15~18%，同文件 gzip 74.41KB vs brotli 64.88KB），gzip 值仍随报告输出作参考；历史 gzip 数值不再可比。去重专项三阶段完成：①9 页样板提取至 `src/scripts/_page-kit.ts`；②42 页 codemod 全量迁移 kit + 统一外置管线（dist 无 Astro 打包 JS）；③basic-page 收尾去重（requireEl/copyText/flashButton 三副本→kit import，附带获得收藏/最近使用注入）+ 6 新工具经 metafile 审计无新增可去重空间（大头为 61 独立引擎真实代码）；**新增代码不得再使 JS 增长，若确需增长（如 P1-7② theme.js +0.5KB）须在此登记并核 brotli 总量**（注：此旧口径要求自 2026-09-21 起由「每页 JS 预算模型」取代，全站总量仅作 120KB 内联回归护栏）**二期瘦身（P6 后，2026-09-21）**：二维码容量表/ASCII 对照表/高温津贴省份预设三工具数据外置为 `public/data/{qr-code-table,ascii-table,heat-subsidy-presets}.json`（.astro 构建期 readFileSync 同源读取、页面 lazy fetch、qr 引擎改 `generateQrCode(text, level, table)` 数据参数化 + `normalizeQrTable` 类型守卫规整）；brotli 118.51→117.72KB（净 −0.79KB，数字/中文表 brotli 压缩率极高致收益低于预期）；browser 实测 7/7 PASS；brotli 分布审计：105 文件最大 3.56KB（qr-code 6.58/8KB per-page）、96 页平均 ~1.1KB、分布扁平无单点肥肉，**当前架构已达结构下限，无剩余削减空间**；护栏余量 2.28KB，下一批次（转换器量产出）触及 120KB 时按护栏语义（数据内联回归守卫、非性能预算）评估上调至 130KB 并在此登记**（已执行：P2 批次 8 个物理量换算器后全站 123.78KB，护栏 120→130KB 已同步 `scripts/check-bundle-size.mjs` 的 TOTAL_GUARDRAIL 与注释，2026-09-21）**；**外部清单导入批次（2026-09-21）**：导入外部工具清单 32 个工具（Dev 8/图片 4/PDF 6 占位/AI 3/游戏 2/SEO 5/法律 4），全站 169.05KB，护栏经 130→140（法律 4 工具）→175KB 两级上调（语义不变：数据内联回归守卫；每页实载仍 ~1.2KB，最差 qr-code-cn 6.65/8KB）；HTTP 状态码 61 条/HTML 实体 226 项/AI 提示词 36 条按数据外置模式放 `public/data/` 懒加载
...
- [ ] P1-10（新增·人工）legal 三页（隐私政策/用户协议/免责声明）为 AI 生成骨架草稿，全站 noindex；须法务审核通过后移除 noindex 并补全第三方服务清单与联系方式｜预估：人工｜🔄 2026-09-20 阻塞项拆解：①**CI 首跑阻塞已解除**——GitHub Actions run #24（commit 11cd400）conclusion=success，verify+build+smoke+产物断言全绿，后续每次 push 自动执行；②剩余阻塞为纯人工项：法务审核三页文本 + 域名定稿后替换 `privacy@example-calculator.cn` 占位邮箱并补第三方服务清单（当前站点零第三方运行时依赖，广告接入后需更新）
- [ ] P2-13（新增·待域名）域名定稿后提交搜索引擎收录：Bing Webmaster Tools + Google Search Console 验证站点、提交 sitemap-index.xml、收录监控启动｜预估 0.5 天（人工）｜⏳ 前置：域名 + 部署环境确定
