...
- [ ] P1-7② 暗色模式第二期（手动切换开关）｜预估 0.5 天｜📋 待人工批复 FOUC + CSP 方案：localStorage + 外置 theme-init.js（不能内联早置脚本，违反 CSP `script-src 'self'`），存在 FOUC 窗口需人工评估接受度。验收：切换按钮 ≥44px 触控目标、aria-pressed 状态、`prefers-color-scheme` 默认跟随可被手动覆盖
