...
7. **质量门禁**：`pnpm verify` = typecheck（strict）+ eslint（0 error 门禁，`no-explicit-any`=error）+ vitest 644 用例 + prettier + mcp:check + bundle:check（**每页 JS 预算模型**：page script+其 chunk ≤8KB / 全站公共脚本 ≤4KB / 全站 JS 总和护栏 ≤120KB / CSS ≤30KB，brotli 口径）；`pnpm verify:dist` 追加 build 后 12 组冒烟断言
